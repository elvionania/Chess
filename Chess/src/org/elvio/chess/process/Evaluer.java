...
import org.elvio.chess.util.BoardEvaluationComparator;
...
public class Evaluer { 
	...
	public static BoardEvalue negaMax(int profondeur, Board board, Byte couleur, int numeroDuCoup, FonctionEvaluation algoDEvaluation, Integer maxParent, boolean test, Temps tempsDuJoueur){
		if (profondeur == 0){
			...
			if(Piece.isMemeCouleur((piece = board.get(position)), couleur)){
				...
				for(int coup : Piece.getPositionsJouables(position, piece, board)){
					...
					etat = BoardUtils.getBoardApresUnCoup(position, piece, coup, board, etat);
						...
						BoardEvalue negamax = negaMax((profondeur - 1), enfant, Piece.inverseCouleur(couleur), numeroDuCoup, algoDEvaluation, (meilleurBoard==null?null:(facteur*max)), enTest, tempsDuJoueur);
						Integer score;
						if(negamax != null){
    ...
    // probleme a resoudre pour la profondeur et le sacrifice!!!
     ...
     private static TreeMap<Board, BoardEvalue> fillTube(String board, Byte couleur, int numeroDuCoup, FonctionEvaluation algoDEvaluation, int facteur) {
        ...
        for(Map.Entry<Board, BoardEvalue> entry : tubeDEvaluationCourant.entrySet()){
            ...
            for(int position : boardCourant.getPositions()){
